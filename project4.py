...
image=0
entries = os.scandir(r"Resources/")
for f_name in entries:
    if not f_name.is_dir():
        if f_name.name.endswith('.png'):
            print(f_name)
            image=f_name

def main(image):
    global src
    src = cv.imread(cv.samples.findFile(r"Resources/"+image))
    if src is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image', help='Path to input image.', default='lena.png')
